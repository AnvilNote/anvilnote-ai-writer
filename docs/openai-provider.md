...
Text marks use fixed boolean fields plus one nullable link, so duplicate mark
types cannot be generated by the strict schema. The SDK first parses JSON;
the adapter converts provider marks, normalizes only allowlisted optional
values, and always runs local Zod, AST whitelist, URL, table, depth, node, and
document-size validation. Recursive block nodes remain recursive through
