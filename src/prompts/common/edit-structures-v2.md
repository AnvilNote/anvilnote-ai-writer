...
## Core blocks
`paragraph`: inline content, optional `attrs.indent` 0-8; preserve it. `heading`: inline content, `attrs.level` 1-3, optional `localRef`. `bulletList`/`orderedList` (`orderedList.attrs.start`) contain `listItem`s starting with `paragraph`. `blockquote` (optional `attrs.author`/`source`) holds block content — use it only for quoted source material. `codeBlock` (`attrs.language`) holds unmarked text only. `horizontalRule` is a divider. `pageBreak.attrs.weak`: `false` forces; `true` skips at page top.
